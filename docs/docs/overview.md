...
Project sync preserves unmanaged Vault content by default. A valid `todoist_task_id` identifies a managed task note, while only folders actually created and recorded by Tasks Bridge are eligible for empty-folder cleanup. User-created Bases, notes, attachments, and folders can coexist inside a mapping without preventing Todoist changes from moving or deleting the managed task notes.

Its custom **Tasks List** Base view rebuilds Project → Section → Task → Subtask hierarchy, lets each view choose any project as its root, and continues to honor native Base filters, result limits, sorting, grouping, and property order. Task rows, toolbar counts, the collapsible **Project overview**, its GitHub-style daily completion heatmap, and per-project progress all derive from the same current Base result under the selected root. Explicit actions edit, complete, or reopen the Todoist task before Project sync refreshes the Markdown projection.
...
- The **Tasks List** Base view adds a native-styled hierarchical workspace with arbitrary project roots, Base-result-scoped statistics, a collapsible overview, a daily completion heatmap, and controlled server-backed task actions.
